...
public class Snake extends JFrame {
    ...
    Point snake; // Variable para la snake
    int widthPoint = 10; // Ancho del punto
    int heightPoint = 10; // Alto del punto
    ImagenSnake imagenSnake; //Instancia de la clase ImagenSnake para la imagen
    String direccion = "RIGHT"; // Cadena que indica la direccion de la snake
    long frequency = 50; // Frecuencia con la que se refresca
    boolean gameOver = false; // Booleano que indica si el juego acabo
    Point comida; // Variable para la comida
    ArrayList<Point> listaPosiciones = new ArrayList<Point>(); // Guarda el tamaño de la snake
    int longitud = 2;
    ...
    public void actualizar() {
        ...
        if ((snake.x > (comida.x - 10) && snake.x < (comida.x + 10)) && (snake.y > (comida.y - 10) && snake.y < (comida.y + 10))) {
            listaPosiciones.add(0, new Point(snake.x, snake.y));
            System.out.println("Tamaño de la lista: " + listaPosiciones.size());
            generarComida();
        }
        imagenSnake.repaint();
    }

    // Genera comida en posiciones aleatorias
    ...
    // Realizara las acciones segun que tecla presionemos 
    ...
    // Dibujara la snake en la ventana
    public class ImagenSnake extends JPanel {
        ...
        public void paintComponent(Graphics g) {
            ...
            if (gameOver) {
                ...
                g.setColor(new Color(28, 100, 21));
            }
            g.fillRect(0, 0, width, height);
            g.setColor(new Color(131, 255, 51));
            ...
            if (gameOver) {
                ...
                g.drawString("PUNTOS: " + (listaPosiciones.size() - 1), 300, 240);
                g.setFont(new Font("TimesRoman", Font.BOLD, 20));
                g.drawString("Presione N para empezar nuevo juego", 100, 320);
                g.drawString("Presione ESC para salir", 100, 340);
            }
        }
    }
